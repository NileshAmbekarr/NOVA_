...
// Native date helpers (replaces moment) (#1)
const fmtDate  = () => new Date().toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata', year: 'numeric', month: '2-digit', day: '2-digit' })
const fmtTime  = () => new Date().toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit', hour12: true })
const fmtDay   = () => new Date().toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata', weekday: 'long' })
const fmtMonth = () => new Date().toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata', month: 'long' })
...
export const updateAssistant = async (req, res) => {
   try {
      ...
      let assistantImage;
      if (req.file) {
         // Use buffer — multer now uses memoryStorage (#5)
...
export const askToAssistant = async (req, res) => {
   try {
      ...
      switch (type) {
         case 'get-date': {
            const resp = `current date is ${fmtDate()}`
            saveHistory(type, resp)
            return res.json({ type, userInput: gemResult.userInput || command, response: resp });
         }
         case 'get-time': {
            const resp = `current time is ${fmtTime()}`
            saveHistory(type, resp)
            return res.json({ type, userInput: gemResult.userInput || command, response: resp });
         }
         case 'get-day': {
            const resp = `today is ${fmtDay()}`
            saveHistory(type, resp)
            return res.json({ type, userInput: gemResult.userInput || command, response: resp });
         }
         case 'get-month': {
            const resp = `current month is ${fmtMonth()}`
            saveHistory(type, resp)
            return res.json({ type, userInput: gemResult.userInput || command, response: resp });
         }
         // ── Note Save ──
         case 'note-save': {
            ...
            if (noteKey && noteValue) {
               // Build updated notes array from the lean snapshot, then persist via findByIdAndUpdate (#6)
               const updatedNotes = [...userNotes]
               const existingIdx = updatedNotes.findIndex(n => n.key.toLowerCase() === noteKey.toLowerCase())
               if (existingIdx >= 0) {
                  updatedNotes[existingIdx] = { ...updatedNotes[existingIdx], value: noteValue, createdAt: new Date() }
               } else {
                  updatedNotes.push({ key: noteKey, value: noteValue, createdAt: new Date() })
               }
               const trimmed = updatedNotes.slice(-50)
               // Fire-and-forget, same pattern as saveHistory
               User.findByIdAndUpdate(req.userId, { notes: trimmed })
                  .catch(err => console.warn('note-save error:', err.message))
            }
